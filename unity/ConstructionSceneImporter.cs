...
[Serializable]
public class CEXOUnityAssetCatalogData
{
    public List<CEXOUnityAssetCatalogEntry> entries;
}

[Serializable]
public class CEXOUnityAssetCatalogEntry
{
    public string name;
...
public class ConstructionSceneImporter : MonoBehaviour
{
    ...
    public string streamingAssetsJson = "demo_standard_unity_scene.json";

    [Header("Asset Catalog JSON")]
    [Tooltip("Use an editable catalog to remap scene prefab names/paths to prefabs in this Unity project.")]
    public bool useAssetCatalogOverrides = true;
    [Tooltip("Optional absolute path to an editable CEXO asset catalog JSON outside the Unity Assets folder.")]
    public string externalAssetCatalogPath;
    [Tooltip("Optional catalog JSON asset. Used if External Asset Catalog Path is empty.")]
    public UnityEngine.Object assetCatalogAsset;
    [Tooltip("Fallback catalog file name inside Assets/StreamingAssets.")]
    public string streamingAssetsCatalogJson = "cexo_asset_catalog.json";
    ...
    public bool createEntranceMarkers = false;
    ...
    public bool createFootprintMarkers = false;
    ...
    private readonly Dictionary<string, CEXOUnityAssetCatalogEntry> catalogByName = new Dictionary<string, CEXOUnityAssetCatalogEntry>(StringComparer.OrdinalIgnoreCase);
    private readonly Dictionary<string, CEXOUnityAssetCatalogEntry> catalogBySourcePath = new Dictionary<string, CEXOUnityAssetCatalogEntry>(StringComparer.OrdinalIgnoreCase);
    ...
    private void LoadAssetCatalogOverrides()
    {
        catalogByName.Clear();
        catalogBySourcePath.Clear();

        if (!useAssetCatalogOverrides)
        {
            return;
        }

        string catalogText = ResolveAssetCatalogText(out string sourceName);
        if (string.IsNullOrEmpty(catalogText))
        {
            return;
        }

        CEXOUnityAssetCatalogData catalog = JsonUtility.FromJson<CEXOUnityAssetCatalogData>(catalogText);
        if (catalog == null || catalog.entries == null)
        {
            Debug.LogWarning($"Asset catalog could not be parsed or has no entries: {sourceName}");
            return;
        }

        foreach (CEXOUnityAssetCatalogEntry entry in catalog.entries)
        {
            if (entry == null)
            {
                continue;
            ...
            {
                catalogByName[entry.name.Trim()] = entry;
            }

            if (!string.IsNullOrWhiteSpace(entry.source_path))
            {
                catalogBySourcePath[NormalizeAssetPath(entry.source_path)] = entry;
            }

            if (!string.IsNullOrWhiteSpace(entry.path))
            {
                string fileName = Path.GetFileNameWithoutExtension(entry.path);
                if (!string.IsNullOrWhiteSpace(fileName) && !catalogByName.ContainsKey(fileName))
                {
                    catalogByName[fileName] = entry;
                }
            }
        }

        Debug.Log($"Loaded {catalog.entries.Count} asset catalog entries from {sourceName}.");
    }

    private string ResolveAssetCatalogText(out string sourceName)
    {
        if (!string.IsNullOrWhiteSpace(externalAssetCatalogPath))
        {
            string expandedPath = Environment.ExpandEnvironmentVariables(externalAssetCatalogPath.Trim());
            sourceName = Path.GetFileName(expandedPath);
            if (File.Exists(expandedPath))
            {
                return File.ReadAllText(expandedPath);
            ...
            Debug.LogWarning($"External asset catalog path does not exist: {expandedPath}");
            return null;
        }

        if (assetCatalogAsset != null)
        {
            sourceName = assetCatalogAsset.name;
            if (assetCatalogAsset is TextAsset textAsset)
            {
                return textAsset.text;
            }

#if UNITY_EDITOR
            string assetPath = AssetDatabase.GetAssetPath(assetCatalogAsset);
            if (!string.IsNullOrEmpty(assetPath))
            {
                string absolutePath = Path.Combine(Directory.GetCurrentDirectory(), assetPath);
                if (File.Exists(absolutePath))
            ...
            Debug.LogWarning($"Assigned Asset Catalog Asset is not readable as text: {assetCatalogAsset.name}");
            return null;
        }

        string path = Path.Combine(Application.streamingAssetsPath, streamingAssetsCatalogJson);
        sourceName = Path.GetFileName(path);
        return File.Exists(path) ? File.ReadAllText(path) : null;
    }
    ...
    private string ResolveSceneJsonPath()
    {
        ...
        {
            "demo_standard_unity_scene.json",
    ...
    {
        CEXOUnityAssetCatalogEntry prefabOverride = FindCatalogOverride(obj.prefab);
        List<PrefabChildData> children = prefabOverride != null && prefabOverride.children != null && prefabOverride.children.Count > 0
            ? prefabOverride.children
            : obj.prefab.children;
        bool catalogReplacesWithSinglePrefab = prefabOverride != null && !string.IsNullOrWhiteSpace(prefabOverride.path);

        if (!catalogReplacesWithSinglePrefab && children != null && children.Count > 0)
        {
            return InstantiateCompoundSceneObject(obj, parent, children);
        }

        string prefabPath = ResolvePrefabPath(obj.prefab);
        GameObject prefab = LoadPrefab(prefabPath);
        if (prefab == null)
        {
            Debug.LogWarning($"Prefab not found for {obj.id}: {prefabPath}. Creating placeholder cube.");
    ...
    private GameObject InstantiateCompoundSceneObject(SceneObjectData obj, Transform parent, List<PrefabChildData> children)
    {
        ...
        foreach (PrefabChildData child in children)
        {
            string childPath = ResolvePrefabPath(child.name, child.path, obj.prefab.construction_class);
            GameObject childPrefab = LoadPrefab(childPath);
            if (childPrefab == null)
            {
                Debug.LogWarning($"Compound child prefab not found for {obj.id}: {childPath}");
                continue;
            }
    ...
    private CEXOUnityAssetCatalogEntry FindCatalogOverride(PrefabData prefab)
    {
        if (prefab == null || !useAssetCatalogOverrides)
        {
            return null;
        }

        if (!string.IsNullOrWhiteSpace(prefab.name) && catalogByName.TryGetValue(prefab.name.Trim(), out CEXOUnityAssetCatalogEntry byName))
        {
            return byName;
        }

        string normalizedPath = NormalizeAssetPath(prefab.path);
        if (!string.IsNullOrWhiteSpace(normalizedPath) && catalogBySourcePath.TryGetValue(normalizedPath, out CEXOUnityAssetCatalogEntry bySource))
        {
            return bySource;
        }

        string fileName = Path.GetFileNameWithoutExtension(prefab.path);
        if (!string.IsNullOrWhiteSpace(fileName) && catalogByName.TryGetValue(fileName, out CEXOUnityAssetCatalogEntry byFileName))
        {
            return byFileName;
        }
    ...
    private CEXOUnityAssetCatalogEntry FindCatalogOverride(string name, string path)
    {
        if (!useAssetCatalogOverrides)
        {
            return null;
        ...
        if (!string.IsNullOrWhiteSpace(name) && catalogByName.TryGetValue(name.Trim(), out CEXOUnityAssetCatalogEntry byName))
        {
            return byName;
        }
    ...
    private string ResolvePrefabPath(PrefabData prefab)
    {
        if (prefab == null)
        {
            return "";
        ...
        CEXOUnityAssetCatalogEntry entry = FindCatalogOverride(prefab);
    ...
    private string ResolvePrefabPath(string name, string path, string constructionClass)
    {
        CEXOUnityAssetCatalogEntry entry = FindCatalogOverride(name, path);
        if (entry != null && !string.IsNullOrWhiteSpace(entry.path))
        {
            return entry.path.Trim();
        }
    ...
    private void ClearExistingRoot()
    {
        ...
        foreach (GameObject obj in allObjects)
        {
            ...
            if (IsGeneratedRootName(obj.name))
            {
                rootsToDestroy.Add(obj);
            }
        }
    ...
    private bool IsGeneratedRootName(string objectName)
    {
        ...
        if (objectName.Equals(generatedRootName, StringComparison.Ordinal))
        {
            return true;
        }

        return objectName.StartsWith($"{generatedRootName}_", StringComparison.Ordinal);
    }
